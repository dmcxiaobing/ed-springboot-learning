...
import com.david.utils.AES;
import com.david.utils.CommonUtils;
import com.david.utils.EncryUtil;
import com.david.utils.RSA;
...
//		server();
		...
		ResponseEntity entity = CommonUtils.toBean(params, ResponseEntity.class);
//
	 ...
	 * 服务端响应数据(加密之后)
	 */
	private static void serverResponse(ResponseEntity entity) throws Exception {
	 ...
	 * @throws Exception
	 */
	private static void clientDecode() throws Exception {
//		Req.data = "";
//		Req.encryptkey = "TZFYvuaH3wVJwHGwZOXliXyTMFzD/OyyqTGphKbbTviJjJ+TemViRFhM250DXiQ8o1+mAC7Y6aw8YiYMMWx6GRBuy8pQbfD8TM+wLEau7myi9dSu2Bc+pJ3fVm8vBoLte4EGV3cmXf8ZVJuJQXyTGQjl3PIZEFJ2JkCo30NTuGs=";
		// 验签
		boolean passSign = EncryUtil.checkDecryptAndSign(Req.data, Req.encryptkey, serverPublicKey, clientPrivateKey);

		if (passSign) {
			// 验签通过
			String aeskey = RSA.decrypt(Req.encryptkey, clientPrivateKey);
			String data = AES.decryptFromBase64(Req.data, aeskey);
			System.out.println("data:::::" + data);
			JSONObject jsonObj = JSONObject.parseObject(data);
			String userid = jsonObj.getString("userid");
